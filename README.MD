...
**Critical Insight**: Improving retrieval architecture (BM25 → semantic embeddings) yielded **50% relative improvement** for Te Reo Māori, while sophisticated budget allocation provided **zero benefit** when retrieval quality was already high.
...
```bash
# Clone repository
git clone https://github.com/yourusername/kaitiaki-planner.git
cd kaitiaki-planner
...
# Build embeddings (one-time, ~3 minutes)
...
```bash
# Terminal 1: Start embedding retriever
cd notebook
...
- **[Statistical Analysis](outputs/statistical_tests.txt)** - Full ANOVA and t-test results
...
This is an academic capstone project. Contributions welcome for:
- Expanding corpus to other low-resource languages
- Improving hybrid retrieval approaches
...
@misc{kaitiaki2025,
  author = {Alex},
  title = {Kaitiaki-Planner: Budget-Aware RAG for Multilingual Fairness},
  year = {2025},
  publisher = {GitHub},
  url = {https://github.com/yourusername/kaitiaki-planner}
}
```
